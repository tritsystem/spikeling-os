...
/// MILESTONE 53: a NINTH hardcoded, boot-time-created process slot --
...
/// a fork() source" pattern exactly.
static FAULT_TEST_PROCESS: Mutex<Option<Process>> = Mutex::new(None);
pub(crate) const FAULT_TEST_PROCESS_ID: u8 = 9;

/// MILESTONE 37: real, dynamic PID allocation for `fork()`-created
/// children. PIDs 1-9 stay permanently reserved for the seven
/// pre-existing hardcoded/loaded-file ids above (1/2/3/4/5, with 6-9 now
/// ALL used: Milestones 41/43/53 claim 6, 7, 8, and 9 respectively -- no
/// more headroom left in this range) so a forked child's PID can never
/// collide with any of them; PROCESS_TABLE's own slot `i` is always PID
/// `PID_TABLE_BASE + i`.
...
pub fn run(id: u8) -> Result<(), &'static str> {
    let slot = match id {
        ...
        _ => {
            return Err(
                "no such process -- use 1, 2, 4 (FDTEST_PROCESS_ID), 5 (FORK_TEST_PROCESS_ID), 6 (SIGSEGV_TEST_PROCESS_ID), 7 (SIGKILL_TEST_PROCESS_ID), 8 (WAITSTATUS_TEST_PROCESS_ID), or 9 (FAULT_TEST_PROCESS_ID)",
            );
        }
    };
